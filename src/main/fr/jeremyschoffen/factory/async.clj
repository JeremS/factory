(ns fr.jeremyschoffen.factory.async
  (:require
    [fr.jeremyschoffen.factory.core :as core]
...
(defn make-combine-mixed-map
  "Similar to [[make-combine-map]] except that the function created here
  accepts a map whose values may not be promises.
...
(defn make-compute-async
  ...
  (fn compute-async [bb-id bb deps current-value]
    (-> deps
        combine-mixed-map
        (then (fn [realized-deps]
                (compute bb-id bb realized-deps current-value))))))
